...
class RoutingCubit extends Cubit<RoutingState> {
  ...
  void finish() {
    ...
    if (state is RoutingMultiRoute) {
      ...
      if (stateRoute.routes.route.length ==
          stateRoute.currentDestinationIndex + 1) {
        emit(state.finish());
        return;
      }

      emit(state.multiRouting(stateRoute.routes, stateRoute.connectors,
          stateRoute.currentDestinationIndex + 1));
    }
  }
  ...
  void routeToAll(UiNode start, List<CategoryModel> destinations) {
    ...
    final destinationNodes = shelves
        .where((e) => destinations.contains(e.connector.category))
        .map((e) => e.connector.node)
        .toList();

    final RouteToAllResult<UiNode>? result =
        graph.routeToAll(start, destinationNodes);
    if (result == null) {
      debugCubit
          ?.addLog('No route found to visit all categories $destinations');
      return;
    }

    emit(state.multiRouting(
        result, shelves.map((e) => e.connector).toList(), 0));
  }
  ...
  void routeTo(UiNode start, CategoryModel destination) {
    ...
    final destinationConnector = shelves
        .firstWhere((element) => element.connector.category == destination)
        .connector;

    final RouteToResult<UiNode>? result =
        graph.routeTo(start, destinationConnector.node);
    if (result == null) {
